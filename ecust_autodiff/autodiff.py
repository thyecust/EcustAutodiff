...
class MatMulOp(Op):
    ...
    def gradient(self, node, output_grad):
        ...
        """DONE: Your code here"""
        if node.matmul_attr_trans_A:
            grad_A = matmul_op(node.inputs[1], output_grad, trans_A=node.matmul_attr_trans_B, trans_B=True)
        else:
            grad_A = matmul_op(output_grad, node.inputs[1], trans_B=(not node.matmul_attr_trans_B))
        if node.matmul_attr_trans_B:
            grad_B = matmul_op(output_grad, node.inputs[0], trans_A=True, trans_B=node.matmul_attr_trans_A)
        else:
            grad_B = matmul_op(node.inputs[0], output_grad, trans_A=(not node.matmul_attr_trans_A))
    ...
    for node in reverse_topo_order:
        ...
        for i in range(len(node.inputs)):
            ...
            if input_node in node_to_output_grads_list:
                ...
                node_to_output_grads_list[input_node] = [grad]
        for (node, grads) in node_to_output_grads_list.items():
...
##############################
#### MNIST Helper Methods #### 
##############################

def softmax(y):
    b = y - np.max(y, axis=1, keepdims=True)
    exp_b = np.exp(b)
    res = exp_b / np.sum(exp_b, axis=1, keepdims=True)
    return res
...
class SoftmaxCrossEntropyOp(Op):
    ...
    def compute(self, node, input_vals):
        ...
        cross_entropy = np.mean(
            -np.sum(y_ * np.log(np.clip(softmax_,1e-12,1e100)), axis=1), keepdims=True)
...
softmax_xentropy_op = SoftmaxCrossEntropyOp()
